#🌥️ MOON YOON SEONG

Cloud & Infrastructure Engineer
데이터 흐름을 이해하고, 안정적인 인프라를 설계하는 것에 관심을 두고 있습니다.
Confluent Kafka · AWS · Linux · Monitoring 기반 프로젝트를 통해
클라우드 환경에서의 운영 구조와 데이터 파이프라인을 경험했습니다.

<br>
🛠️ Tech Focus

Cloud: AWS, GCP

Infrastructure: Linux, Shell, Docker

Data Pipeline: Confluent Kafka, Kafka Cluster, 이벤트 스트림 처리

Monitoring: Grafana, Prometheus, CloudWatch

Backend: Spring Boot, Node.js

Language: Python, Java

<br>

🌐 What I'm Interested In

대규모 로그 환경에서의 데이터 수집·인덱싱·모니터링

클라우드 기반 시스템의 확장성과 안정성 설계

Observability(관측성) 기반 운영 체계 구축

무중단 Kafka Cluster 및 실시간 데이터 처리

Splunk 기반 분석 및 운영 자동화

<br>

<br>
📊 Stats & Algorithm
[![Solved.ac Profile](http://mazassumnida.wtf/api/v2/generate_badge?boj=moonys2468)](https://solved.ac/moonys2468/)
<br>

[![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=yohan050605)](https://github.com/anuraghazra/github-readme-stats)
